...
export default function LiveVerificationVisual() {
  ...
  return (
    <div className="relative mx-auto w-full max-w-5xl pt-4">
      {/* Subtle atmospheric red glow behind visual */}
      <div className="absolute -inset-1 rounded-3xl bg-gradient-to-r from-neonRed/20 via-neonRed-deep/15 to-neonRed/20 blur-2xl opacity-60" />

      <div className="relative overflow-hidden rounded-3xl border border-graphite-border bg-graphite-surface/95 backdrop-blur-2xl shadow-2xl">
        <RedSweepBorder />
        ...
        <div className="flex items-center justify-between border-b border-graphite-border px-6 py-4 bg-graphite-sub">
          <div className="flex items-center gap-3">
            <div className="flex gap-1.5">
              <div className="h-3 w-3 rounded-full bg-[#241014]" />
              <div className="h-3 w-3 rounded-full bg-[#241014]" />
              <div className="h-3 w-3 rounded-full bg-[#241014]" />
            </div>
        ...
        <div className="relative border-b border-graphite-border bg-graphite-bg/70 p-3 overflow-x-auto no-scrollbar">
          {/* Animated path line glow */}
          <div className="absolute top-1/2 left-6 right-6 h-[2px] bg-[#241014] -translate-y-1/2 hidden md:block" />
          <motion.div
            animate={{
              left: `${(activeStep / (steps.length - 1)) * 80 + 8}%`,
            }}
            transition={{ duration: 0.5, ease: "easeInOut" }}
            className="absolute top-1/2 h-[2px] w-24 bg-gradient-to-r from-neonRed via-neonRed-bright to-transparent -translate-y-1/2 hidden md:block pointer-events-none"
          />

          <div className="flex justify-between min-w-[700px] relative z-10 px-2 gap-2">
            {steps.map((step, idx) => {
              ...
              return (
                <button
                  ...
                  className={`flex-1 flex flex-col items-center gap-2 p-2 rounded-xl border transition-all text-center relative ${
                    isActive
                      ? "bg-graphite-elevated border-neonRed/60 shadow-[0_0_20px_rgba(255,23,68,0.2)]"
                      : isPassed
                      ? "bg-graphite-surface/60 border-graphite-border text-muted-foreground"
                ...
                >
                  <div
                    className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-lg font-mono text-xs font-bold transition-colors ${
                      isActive
                        ? "bg-neonRed text-foreground shadow-red-glow"
                        : isPassed
                        ? "bg-graphite-elevated text-neonRed-bright border border-graphite-border-sec"
                        : "bg-graphite-sub text-muted-foreground"
                    }`}
                  >
                    {isPassed ? <CheckCircle2 className="h-4 w-4 text-neonRed-bright" /> : `0${idx + 1}`}
                  </div>

                  <p
                    className={`font-mono text-[9px] uppercase tracking-wider hidden sm:block ${
                      isActive ? "text-neonRed-bright font-bold" : "text-muted-foreground"
                    }`}
                  >
                    {step.tag}
                  </p>
        ...
        {/* Step Content Window with Framer Motion Stagger */}
          ...
          <AnimatePresence mode="wait">
            ...
            >
              ...
              <div className="flex items-center justify-between">
                <span
                  className={`font-mono text-xs font-bold tracking-widest px-3.5 py-1.5 rounded-full border bg-graphite-elevated flex items-center gap-2 ${current.color} ${current.borderColor}`}
                >
                  <current.icon className="h-3.5 w-3.5 animate-pulse" />
                  {current.tag}
                </span>
                <span className="font-mono text-xs text-muted-foreground">
                  STATUS: {activeStep === steps.length - 1 ? 'COMPLETE' : 'PROCESSING'}
                </span>
              </div>

              {/* Dynamic Step Render */}
              {current.id === "input" && (
                <div className="p-5 rounded-2xl bg-graphite-bg border border-graphite-border space-y-2">
                  <p className="font-mono text-xs text-neonRed-label uppercase tracking-wider">INPUT CLAIM</p>
                ...
                <div className="space-y-3">
                  {current.content.extracted?.map((ext, i) => (
                    <motion.div
                      ...
                      className="flex items-center justify-between p-4 rounded-2xl bg-graphite-bg border border-graphite-border font-mono text-sm"
                    >
                      <span className="text-foreground">"{ext.text}"</span>
                      <span className="text-neonRed-bright text-xs">CONFIDENCE: {ext.confidence}%</span>
                    </motion.div>
                  ))}
                </div>
              )}
                ...
                <div className="space-y-3">
                  {current.content.sources?.map((src, i) => (
                    <motion.div
                      ...
                      animate={{ opacity: 1, x: 0 }}
                      transition={{ delay: i * 0.12 }}
                      className="flex items-center justify-between p-3.5 rounded-2xl bg-graphite-bg border border-graphite-border font-mono text-xs hover:border-neonRed/40 transition-colors"
                    >
                      <div className="flex items-center gap-3">
                      ...
                      <span className="text-neonRed bg-neonRed/10 px-2 py-0.5 rounded border border-neonRed/20">
                        {src.time}
                      </span>
                    </motion.div>
                  ))}
                ...
                <div className="grid gap-3 sm:grid-cols-2">
                  {current.content.matches?.map((match, i) => (
                    <motion.div
                      ...
                      className="p-4 rounded-2xl bg-graphite-bg border border-graphite-border space-y-2"
                    >
                      <div className="flex items-center justify-between">
                        <span className="font-display text-xs font-bold text-foreground truncate">
                          {match.title}
                        </span>
                        <span className="font-mono text-[10px] text-verificator-verified bg-verificator-verified/10 px-2 py-0.5 rounded border border-verificator-verified/20">
              ...
              {current.id === "evaluate" && (
                <div className="flex flex-col gap-4 p-5 rounded-2xl bg-graphite-bg border border-graphite-border">
                  {current.content.metrics?.map((metric, i) => (
                    <div key={metric.label} className="space-y-1.5">
                      ...
                      <div className="h-1.5 w-full bg-graphite-elevated rounded-full overflow-hidden">
                        <motion.div
                          initial={{ width: 0 }}
                          animate={{ width: `${metric.value}%` }}
                          transition={{ delay: i * 0.2, duration: 0.8, ease: "easeOut" }}
                          className="h-full bg-neonRed shadow-red-glow rounded-full"
                        />
                      </div>
                    </div>
                  ))}
              ...
              {current.id === "verdict" && (
                ...
                >
                  ...
                  <div className="flex flex-col items-center justify-center p-4 rounded-2xl bg-graphite-bg border border-verificator-verified/30 min-w-[120px]">
