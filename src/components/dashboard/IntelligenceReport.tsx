...
import { ShieldAlert, ShieldCheck, AlertTriangle, ShieldQuestion, XOctagon, Download, Loader2, AlertCircle, Drama, Sparkles, Flag, Link2, Check } from "lucide-react";
...
export default function IntelligenceReport({ verdict, confidenceScore, confidenceBreakdown, summary, keyFindings, lowSourceDiversity }: IntelligenceReportProps) {
  const { result, url, text, dbSaveState } = useAnalysis();
  const [isGeneratingPdf, setIsGeneratingPdf] = useState(false);
  const [pdfError, setPdfError] = useState<string | null>(null);
  const [flagState, setFlagState] = useState<"idle" | "submitting" | "submitted" | "error">("idle");
  const [shareState, setShareState] = useState<"idle" | "loading" | "copied" | "error">("idle");
  const [shareUrl, setShareUrl] = useState<string | null>(null);

  const analysisId = dbSaveState.analysisId;

  const handleFlagVerdict = async () => {
    if (!analysisId || flagState === "submitting") return;
    setFlagState("submitting");
    try {
      const res = await fetch("/api/feedback", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ analysisId, type: "verdict_wrong" }),
      });
  ...
  const handleShareLink = async () => {
    if (!analysisId || shareState === "loading") return;
    ...
    try {
      ...
      const link = `${window.location.origin}/report/${data.publicSlug}`;
      setShareUrl(link);
  ...
  const getVerdictConfig = (v: string) => {
    switch (v) {
      ...
      case "SATIRE":
        return { color: "text-purple-400", bg: "bg-purple-500/10", border: "border-purple-500/50", icon: Drama };
      default:
  ...
  return (
    <div className="w-full bg-graphite-bg border border-graphite-border rounded-2xl overflow-hidden relative">
      ...
      <div className="p-6 md:p-8 space-y-8">
        {/* AI attribution (§9.5): verdicts are AI-generated and can be wrong — never presented with unqualified authority. */}
        <div className="flex items-start gap-2.5 p-3 rounded-xl bg-graphite-surface border border-graphite-border">
          ...
          <p className="text-[11px] text-muted-foreground leading-relaxed">
            This assessment was generated by an AI multi-agent debate system based on evidence gathered from the live web. It can be wrong — treat it as a research aid, not a final authority, and verify independently before acting on it.
          </p>
        </div>

        {lowSourceDiversity && (
          <div className="flex items-start gap-2.5 p-3 rounded-xl bg-verificator-warning/10 border border-verificator-warning/30">
            <AlertTriangle className="w-3.5 h-3.5 shrink-0 mt-0.5 text-verificator-warning" />
            <p className="text-[11px] text-verificator-warning leading-relaxed">
              Fewer than 2 independent domains corroborated this verdict — confidence has been capped to reflect the limited corroboration (§3.9).
            </p>
          </div>
        )}
        ...
        {confidenceBreakdown && (
          <div>
            ...
            <div className="space-y-3">
              {[
                { label: "SOURCE QUALITY", value: confidenceBreakdown.sourceQuality, max: 35 },
                { label: "EVIDENCE STRENGTH", value: confidenceBreakdown.evidenceStrength, max: 25 },
                { label: "AGENT AGREEMENT", value: confidenceBreakdown.agentAgreement, max: 25 },
                { label: "FRESHNESS", value: confidenceBreakdown.freshness, max: 15 },
              ].map((row) => (
                <div key={row.label} className="flex items-center gap-3">
                  <span className="w-36 shrink-0 font-mono text-[10px] text-muted-foreground uppercase tracking-wider">{row.label}</span>
                  <div className="flex-1 h-2 rounded-full bg-graphite-surface border border-graphite-border overflow-hidden">
                    <div
                      className="h-full bg-neonRed rounded-full transition-all"
                      style={{ width: `${Math.min(100, (row.value / row.max) * 100)}%` }}
                    />
                  </div>
                  <span className="w-14 shrink-0 text-right font-mono text-[10px] text-foreground">{row.value}/{row.max}</span>
                </div>
              ))}
            </div>
            <p className="mt-3 text-[10px] text-muted-foreground font-mono">
              Computed deterministically from source credibility, evidence-verdict alignment, debate-agent agreement, and evidence freshness — not a model's self-reported guess.
            </p>
          </div>
        )}
          ...
          {analysisId && (
            <>
              <button
                ...
                className="h-10 px-4 rounded-xl bg-graphite-bg border border-graphite-border text-xs font-mono tracking-wider font-semibold text-foreground hover:text-neonRed hover:border-neonRed/50 hover:bg-graphite-elevated transition-all flex items-center gap-2 shadow-sm disabled:opacity-50 cursor-pointer"
                title="Generate a public, read-only link to this report"
              >
                {shareState === "loading" ? (
                  <Loader2 className="w-4 h-4 text-neonRed animate-spin" />
                ) : shareState === "copied" ? (
                  <Check className="w-4 h-4 text-verificator-verified" />
                ) : (
                  <Link2 className="w-4 h-4 text-neonRed" />
                )}
                <span>{shareState === "copied" ? "LINK COPIED" : "COPY PUBLIC LINK"}</span>
              </button>

              <button
                type="button"
                onClick={handleFlagVerdict}
                disabled={flagState === "submitting" || flagState === "submitted"}
                className="h-10 px-4 rounded-xl bg-graphite-bg border border-graphite-border text-xs font-mono tracking-wider font-semibold text-foreground hover:text-verificator-warning hover:border-verificator-warning/50 hover:bg-graphite-elevated transition-all flex items-center gap-2 shadow-sm disabled:opacity-50 cursor-pointer"
                title="Flag this verdict as wrong for review (§9.2)"
              >
                <Flag className={`w-4 h-4 ${flagState === "submitted" ? "text-verificator-warning" : "text-verificator-warning"}`} />
                <span>{flagState === "submitted" ? "FLAGGED FOR REVIEW" : "FLAG THIS VERDICT"}</span>
              </button>
            </>
          )}
        </div>
